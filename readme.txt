...
- There are 16 different currencies in the file with no exchange rates given, so I stayed away from any $ comparisons across suppliers.
- Some fields like invoice status and compliance docs are missing on a lot of rows, even ones that were fully delivered. Looks like a real gap in the data rather than something to fill in, so I left those alone.
- This only covers delivery and quality for now. Didn't get to compliance docs, pricing accuracy, or cancellation rate - could be good next additions.
